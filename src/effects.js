...
function flightSpeed(flight) {
  ...
  if (flight?.velocity?.isVector3) {
    return flight.velocity.length();
  }
...
function makeStreakGeometry(
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    seeds[seedIndex] = angle;
    seeds[seedIndex + 1] = radial;
    seeds[seedIndex + 2] = z;
    seeds[seedIndex + 3] = length;
  }
...
export class EffectsSystem {
  constructor(scene) {
    ...
    const streakCount = Math.min(
      ...
      Math.max(
        48,
        CONFIG.effects?.streakCount ||
          120,
      ),
    );

    this.streakDepth =
      CONFIG.effects?.streakDepth || 65;

    this.streakRadius =
      CONFIG.effects?.streakRadius || 10;
    ...
    this.streaks.name =
      'camera-local-speed-streaks';

    this.streaks.frustumCulled =
      false;

    this.streaks.renderOrder = 3;

    this.aircraft =
      new AircraftVisualSystem(scene);

    this.skyDecor =
      new SkyDecorSystem(scene);
  }
  ...
  _attachToCamera(camera) {
    ...
    if (this.streaks.parent) {
      this.streaks.parent.remove(
        this.streaks,
      );
    }
  ...
  _updateStreaks(
    ...
    const positions =
      this.streakGeometry.attributes
        .position.array;

    const seeds =
      this.streakGeometry.userData
        .seeds;
  ...
  update(dt, flight, camera) {
    ...
    /*
     * Keep VR shake extremely small.
     * Ordinary turns do not shake the camera.
     * Only a severe stall produces buffet.
     */
    const shakeLimit =
      CONFIG.effects?.maxVrShake ??
      (0.18 * Math.PI) / 180;
