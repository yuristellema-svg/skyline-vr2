// SKYLINE_LIVE_CITY_GROUNDING_V3
// Locked asset URLs are preserved; this service-worker source revision refreshes them.
// SKYLINE_AIRFIELD_WORLD_CORE_REMAP_V3
// SKYLINE_AIRFIELDS_LIVE_INTEGRATION_V2
// SKYLINE_NAVIGATION_PIN_VISIBLE_V1
// SKYLINE_VR_MENU_MAP_FORWARD_V2
// SKYLINE_VR_UI_ANCHOR_PING_V1
const SHELL_CACHE = 'skyline-biplane-zero-radio-v4-20260718';

const SHELL_FILES = [
  './',
  './index.html',
  './styles.css',
  './manifest.webmanifest',
  './icon.svg',
  './apple-touch-icon.png',
  './src/main.js?v=biplane-zero-radio-v4',
  './src/config.js',
  './src/input.js',
  './src/flightModel.js',
  './src/collision.js',
  './src/camera.js',
  './src/effects.js',
  './src/aircraftVisuals.js?v=biplane-zero-radio-v4',
  './src/skyDecor.js',
  './src/atmosphere.js',
  './src/windAudio.js?v=biplane-zero-radio-v4',
  './src/routeSystem.js',
  './src/sandboxDynamics.js',
  './src/aiTraffic.js',
  './src/cloudField.js',
  './src/contrails.js',
  './src/worldPolish.js?v=biplane-zero-radio-v4',
  './src/livingAirspace/index.js',
  './src/livingAirspace/system.js',
  './src/livingAirspace/constants.js',
  './src/livingAirspace/catalog.js',
  './src/livingAirspace/math.js',
  './src/livingAirspace/quality.js',
  './src/livingAirspace/resources.js',
  './src/livingAirspace/birds.js',
  './src/livingAirspace/traffic.js',
  './src/livingAirspace/clouds.js',
  './src/livingAirspace/depth.js',
  './src/navigationMap/index.js',
  './src/airfields/airfieldCatalog.js',
  './src/airfields/airfieldDiagnostics.js',
  './src/airfields/airfieldGeometry.js',
  './src/airfields/airfieldVisuals.js',
  './src/airfields/landingCapability.js',
  './src/airfields/landingMath.js',
  './src/airfields/lightingPlan.js',
  './src/airfields/obstacleClearance.js',
  './src/airfields/operations.js',
  './src/airfields/terrainFit.js',
  './src/airfields/worldIntegration.js',
  './src/navigation/approachGuidance.js',
  './src/navigation/fieldSelection.js',
  './src/navigation/navigationVisuals.js',
  './src/navigation/radioNavigation.js',
  './src/expansion/landingSystem.js',
  './src/expansion/runwayGuidance.js',
  './src/navigationMap/mapCatalog.js',
  './src/navigationMap/navigationMapSystem.js',
  './src/worldCompletion/index.js',
  './src/worldCompletion/livingAirspaceCatalog.js',
  './src/worldCompletion/localInfrastructureSystem.js',
  './src/worldCompletion/outerDetailSystem.js',
  './src/worldCompletion/settlementManifestAdapter.js',
  './src/settlements/archetypes.js',
  './src/settlements/catalogBuilder.js',
  './src/settlements/collisionCatalog.js',
  './src/settlements/constants.js',
  './src/settlements/descriptor.js',
  './src/settlements/families.js',
  './src/settlements/harbour.js',
  './src/settlements/index.js',
  './src/settlements/landmarks.js',
  './src/settlements/manifest.js',
  './src/settlements/math.js',
  './src/settlements/planner.js',
  './src/settlements/renderPlan.js',
  './src/settlements/sampleCatalog.js',
  './src/settlements/settlementPlacement.schema.json',
  './src/settlements/settlementSystem.js',
  './src/settlements/spatial.js',
  './src/settlements/terrain.js',
  './src/settlements/threeRenderer.js',
  './src/settlements/urbanDesign.js',
  './src/aircraft/a6mZeroShared.js',
  './src/aircraft/a6mZeroExternal.js',
  './src/aircraft/a6mZeroCockpit.js',
  './src/optionalWorld/safeSystem.js',
  './src/optionalWorld/math.js',
  './src/optionalWorld/boostHoops.js',
  './src/optionalWorld/cloudField.js',
  './src/optionalWorld/distantCity.js',
  './src/optionalWorld/aiAircraft.js',
  './src/optionalWorld/audio.js',
  './src/optionalWorld/index.js',
  './src/performanceRuntime.js',
  './src/aircraftFlightProfiles.js',
  './src/aircraft/aircraftVisualShared.js',
  './src/aircraft/ju87StukaExternal.js',
  './src/aircraft/ju87StukaCockpit.js',
  './src/audio/audioMath.js',
  './src/audio/aircraftProfiles.js',
  './src/audio/aircraftEngineAudio.js',
  './src/audio/airflowAudio.js',
  './src/audio/flightWarningLogic.js',
  './src/audio/proceduralWarningVoice.js',
  './src/audio/flightWarningAudio.js',
  './src/audio/boostAudio.js',
  './src/audio/positionalTrafficAudio.js',
  './src/stereo.js',
  './src/vrMenuBeacon.js',
  './src/menu.js',
  './src/hud.js',
  './src/world/world.js',
  './assets/world/world-core-v2-manifest.json',
  './src/worldExpansion/index.js',
  './src/worldExpansion/biomeModel.js',
  './src/worldExpansion/featuresRuntime.js',
  './src/worldExpansion/heightModel.js',
  './src/worldExpansion/layoutCompiler.js',
  './src/worldExpansion/manifest.js',
  './src/worldExpansion/math.js',
  './src/worldExpansion/terrainRuntime.js',
  './src/expansion/radioBeacon.js?v=biplane-zero-radio-v4',
  './src/audio/zeroRadioAudio.js?v=biplane-zero-radio-v4',
  './src/audio/stukaDiveSiren.js?v=biplane-zero-radio-v4',
  './src/aircraft/biplaneCockpit.js',
  './src/aircraft/biplaneExternal.js',
  './src/aircraft/biplaneProfile.js',
  './src/aircraft/biplaneRuntime.js',
  './src/aircraft/biplaneSpecs.js',
  './src/aircraft/biplaneVisualShared.js',
  './assets/audio/zero-radio.mp3',
  './assets/audio/stuka-siren.mp3',
  './vendor/three.module.min.js',
];

self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(SHELL_CACHE)
      .then(cache => cache.addAll(SHELL_FILES))
      .then(() => self.skipWaiting()),
  );
});

self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(key => key !== SHELL_CACHE).map(key => caches.delete(key))))
      .then(() => self.clients.claim()),
  );
});

async function networkFirst(request, fallback = null) {
  const cache = await caches.open(SHELL_CACHE);
  try {
    const response = await fetch(request);
    if (response.ok) await cache.put(request, response.clone());
    return response;
  } catch {
    return (await caches.match(request)) || (fallback ? await caches.match(fallback) : Response.error());
  }
}

self.addEventListener('fetch', event => {
  const request = event.request;
  if (request.method !== 'GET') return;

  const url = new URL(request.url);
  if (url.origin !== self.location.origin) return;

  if (request.mode === 'navigate') {
    event.respondWith(networkFirst(request, './index.html'));
    return;
  }

  if (
    url.pathname.endsWith('.js') ||
    url.pathname.endsWith('.css') ||
    url.pathname.endsWith('.json') ||
    url.pathname.endsWith('.webmanifest') ||
    url.pathname.includes('/assets/world/') ||
    url.pathname.endsWith('.wpk')
  ) {
    event.respondWith(networkFirst(request));
    return;
  }

  event.respondWith(
    caches.match(request).then(cached => cached || networkFirst(request)),
  );
});
